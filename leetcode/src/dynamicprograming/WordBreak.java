...
public class WordBreak {
    ...
    public static boolean helper(int idx, HashSet<String> dict, String s, Boolean[] dp) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (dict.contains(sub) && helper(idx + i+1, dict, s.substring(i + 1), dp)) {
                dp[idx] = true;
                return true;
            }
        }
    ...
    public static boolean wordBreak90(String s, List<String> wordDict) {
    ...
    private static boolean backtracking(int depth, String s, Set<String> wordSet, Boolean[] memo) {
        int n = s.length();
        // accept
        if (depth == n) {
            return true;
